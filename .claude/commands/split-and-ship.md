...
2. **Create a new branch** using conventional naming: `git checkout -b <type>/<short-description>` where type is `feat`, `fix`, `refactor`, `chore`, `docs`, etc. Examples: `feat/multi-prompt-field`, `fix/credit-calculation`, `refactor/provider-factory`
...
After all PRs are created, checkout back to the branch the user was originally on (or `master` if it was a detached HEAD).
...
- Branch names must follow conventional naming: `<type>/<short-kebab-case-description>` (e.g., `feat/multi-prompt-field`, `fix/credit-calculation`).
